...
use emu_abi::memory::{MemProt, PAGE_SIZE, PagePointer, TLB_SIZE};
...
fn iommu_with_sparse_page_protections_and_bytes(
    ...
    for &(page, protections) in protections {
        ...
        if needs_store {
            mmu.store_force(base, &page).unwrap();
...
fn vm_store64_writes_mapped_memory_after_tlb_miss() {
    ...
    assert_eq!(mmu.load64_le(16).unwrap(), 0x0123_4567_89ab_cdef,);
}
...
fn vm_unaligned_store32_uses_iommu_fallback() {
    ...
    assert_eq!(mmu.load32_le(3).unwrap(), 0x89ab_cdef);
}
...
fn vm_cross_page_store64_uses_iommu_fallback() {
    ...
    assert_eq!(mmu.load64_le(start).unwrap(), 0x0123_4567_89ab_cdef,);
}
...
fn vm_store_traps_on_missing_write_permission_and_does_not_modify_memory() {
    ...
    assert_eq!(mmu.load64_le(8).unwrap(), expected);
}
...
fn vm_aligned_fast_path_stores_all_widths_to_page0_nonzero_offsets() {
    ...
    assert_eq!(mmu.load_byte(1).unwrap(), 0xa7);
    assert_eq!(mmu.load16_le(2).unwrap(), 0xb8c9);
    assert_eq!(mmu.load32_le(4).unwrap(), 0xdade_beef);
    assert_eq!(mmu.load64_le(8).unwrap(), 0x0123_4567_89ab_cdef);
}
...
fn vm_tlb_collision_store_does_not_write_through_different_virtual_page_entry() {
    ...
    assert_eq!(
        mmu.load64_le(vm_page_addr(0)).unwrap(),
        0x1111_2222_3333_4444,
    );
    assert_eq!(
        mmu.load64_le(vm_page_addr(alias_page)).unwrap(),
...
fn vm_tlb_collision_store_to_unmapped_page_traps_instead_of_reusing_old_entry() {
    ...
    let page0_before = mmu.load64_le(vm_page_addr(0)).unwrap();
    let mut state = ProcessorState::initial();

    let code = run_with_mmu(builder, &mut state, &mmu);

    assert_memory_trap(code);
    assert_eq!(mmu.load64_le(vm_page_addr(0)).unwrap(), page0_before);
}
...
fn vm_tlb_collision_store_permission_check_uses_colliding_target_page() {
    ...
    let page0_before = mmu.load64_le(vm_page_addr(0)).unwrap();
    let alias_before = mmu.load64_le(vm_page_addr(alias_page)).unwrap();
...
fn vm_tlb_hit_store_uses_page_number_not_zero_for_page1_offset0() {
    ...
    assert_eq!(mmu.load64_le(vm_page_addr(0)).unwrap(), 0);
    assert_eq!(
        mmu.load64_le(vm_page_addr(1)).unwrap(),
...
fn vm_fast_path_store_at_exact_end_of_page_succeeds_for_w16_w32_and_w64() {
    {
        ...
        assert_eq!(
            mmu.load16_le(u64::try_from(start).unwrap()).unwrap(),
            0x1234,
        );
    }

    {
        ...
        assert_eq!(
            mmu.load32_le(u64::try_from(start).unwrap()).unwrap(),
            0x4567_89ab,
        );
    }

    {
        ...
        assert_eq!(
            mmu.load64_le(u64::try_from(start).unwrap()).unwrap(),
...
fn vm_byte_access_at_last_byte_of_page_uses_fast_path_and_succeeds() {
    ...
    assert_eq!(state.x_registers[0], 1);
    assert_eq!(mmu.load_byte(u64::try_from(last).unwrap()).unwrap(), 0xee,);
}
...
fn vm_dynamic_store_can_take_fast_path_and_fallback_path_in_same_compiled_chunk() {
    ...
    assert_eq!(aligned_mmu.load64_le(8).unwrap(), 0x1111_2222_3333_4444,);
    ...
    assert_eq!(fallback_mmu.load64_le(3).unwrap(), 0xaaaa_bbbb_cccc_dddd,);
}
...
fn vm_cross_page_store32_traps_when_second_page_lacks_write_and_does_not_partially_store() {
    ...
    let before1 = mmu
        ...
        .unwrap();
    let before2 = mmu.load_byte(vm_page_addr(1)).unwrap();
    let before3 = mmu.load_byte(vm_page_addr(1) + 1).unwrap();
    ...
    assert_eq!(
        mmu.load_byte(u64::try_from(PAGE_SIZE - 2).unwrap())
            .unwrap(),
        before0,
    );
    assert_eq!(
        mmu.load_byte(u64::try_from(PAGE_SIZE - 1).unwrap())
            .unwrap(),
        before1,
    );
    assert_eq!(mmu.load_byte(vm_page_addr(1)).unwrap(), before2);
    assert_eq!(mmu.load_byte(vm_page_addr(1) + 1).unwrap(), before3);
}
...
fn vm_store_then_load_same_address_in_same_ir_sees_new_value_fast_path() {
    ...
    assert_eq!(mmu.load64_le(32).unwrap(), 0x9988_7766_5544_3322);
}
...
fn vm_store_then_load_same_unaligned_address_in_same_ir_sees_new_value_fallback_path() {
    ...
    assert_eq!(mmu.load64_le(33).unwrap(), 0x8877_6655_4433_2211);
}
...
fn vm_store_traps_when_tlb_miss_fallback_reports_unmapped_page() {
    ...
    assert_memory_trap(code);
    assert_eq!(mmu.load64_le(0).unwrap(), 0);
}
...
fn vm_store_to_executable_page_dirties_icache_page() {
    ...
    assert_eq!(mmu.load64_le(16).unwrap(), 0xfeed_face_cafe_beef);
    assert_eq!(
        drain_dirty_icache_pages(&mmu),
        1,
...
fn repeated_vm_stores_to_same_executable_page_report_one_dirty_page() {
    ...
    assert_eq!(mmu.load64_le(16).unwrap(), 0x1111_2222_3333_4444);
    assert_eq!(mmu.load64_le(24).unwrap(), 0x5555_6666_7777_8888);
    assert_eq!(
        drain_dirty_icache_pages(&mmu),
        1,
...
fn vm_store_to_executable_page1_dirties_only_one_page() {
    ...
    assert_eq!(
        mmu.load64_le(vm_page_addr(1).strict_add(32)).unwrap(),
...
fn vm_unaligned_store64_fallback_dirties_executable_icache_page() {
    ...
    assert_eq!(mmu.load64_le(3).unwrap(), 0xfeed_face_cafe_beef);
    assert_eq!(
        drain_dirty_icache_pages(&mmu),
        1,
