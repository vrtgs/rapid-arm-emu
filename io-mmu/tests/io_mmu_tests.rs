...
fn write_pages_to_new_mmu(mmu: &IoMMU, start: u64, size: usize, mut byte: impl FnMut(u64) -> u8) {
    ...
    if let Some(bytes) = bytes {
        TLB.with_borrow_mut(|tlb| mmu.store_force(tlb, start, &bytes).unwrap())
    }
}
...
thread_local! {
    static TLB: RefCell<Tlb> = const { RefCell::new(Tlb::new()) };
}

fn read_array<const N: usize>(mmu: &IoMMU, vaddr: u64) -> [u8; N] {
    let mut out = [0; N];
    TLB.with_borrow_mut(|tlb| mmu.load(tlb, vaddr, &mut out))
        .unwrap();
    out
}
...
fn is_dirty(mmu: &IoMMU, page: usize) -> bool {
    ...
    let page = TLB
        .with_borrow_mut(|tlb| {
            mmu.get_page(
                tlb,
                PageNumber::from_page_number(BASE_PAGE.strict_add(offset_page)),
            )
        })
        .unwrap();
...
fn new_mmu_faults_non_empty_accesses() {
    ...
    TLB.with_borrow_mut(|tlb| {
        ...
        assert!(mmu.load_into_uninit(&mut *tlb, BASE, &mut one).is_err());
        assert!(mmu.store(&mut *tlb, BASE, &[1]).is_err());

        assert!(mmu.load_byte(&mut *tlb, BASE).is_err());
        assert!(mmu.store_byte(&mut *tlb, BASE, 1).is_err());

        assert!(mmu.load16_le(&mut *tlb, BASE).is_err());
        assert!(mmu.load32_le(&mut *tlb, BASE).is_err());
        assert!(mmu.load64_le(&mut *tlb, BASE).is_err());

        assert!(mmu.store16_le(&mut *tlb, BASE, 0x1234).is_err());
        assert!(mmu.store32_le(&mut *tlb, BASE, 0x1234_5678).is_err());
        assert!(
            mmu.store64_le(&mut *tlb, BASE, 0x1234_5678_9abc_def0)
                .is_err()
        );
    })
}
...
fn zero_length_load_and_store_do_not_require_mapping() {
    ...
    let loaded = mmu
        .load_into_uninit(NoCache, 0x1234_5678, &mut empty)
        .unwrap();
    assert!(loaded.is_empty());
    assert!(mmu.store(NoCache, 0x1234_5678, &[]).is_ok());
}
...
fn map_memory_rejects_unaligned_base_size_ptr_and_overflow() {
    ...
    let rejected_combinations = [
        ...
        (overflow_base, PAGE_SIZE_U64.strict_mul(2)),
    ];
...
#[test]
#[should_panic]
fn map_memory_rejects_mapping_unaligned() {
    let mut mmu = IoMMU::new(CpuFabric::new());
    let pages = 2;
    let mut manager = AllocManager::new(pages);
    let ptr = manager.get_mem().as_mut_ptr();
...
fn nonzero_base_maps_only_requested_page() {
    ...
    TLB.with_borrow_mut(|tlb| {
        assert!(mmu.load_byte(&mut *tlb, 0).is_err());
        assert!(mmu.load_byte(&mut *tlb, page_addr(1)).is_err());
        assert_eq!(mmu.load_byte(&mut *tlb, base).unwrap(), 0xaa);
    });
}
...
fn read_only_page_allows_loads_and_rejects_stores() {
    ...
    TLB.with_borrow_mut(|tlb| {
        ...
        let mut one = [MaybeUninit::<u8>::uninit()];
        assert!(mmu.load_into_uninit(&mut *tlb, BASE, &mut one).is_ok());

        assert!(mmu.store_byte(&mut *tlb, BASE, 1).is_err());
        assert!(mmu.store(&mut *tlb, BASE, &[1]).is_err());
        assert!(mmu.store16_le(&mut *tlb, BASE, 0xbeef).is_err());
        assert!(mmu.store32_le(&mut *tlb, BASE, 0xfeed_beef).is_err());
        assert!(
            mmu.store64_le(&mut *tlb, BASE, 0xfeed_beef_dead_cafe)
                .is_err()
        );
    });
}
...
fn write_only_page_allows_stores_and_rejects_loads() {
    ...
    TLB.with_borrow_mut(|tlb| {
        assert!(mmu.store_byte(&mut *tlb, BASE, 1).is_ok());
        assert!(mmu.store(&mut *tlb, BASE, &[1, 2, 3]).is_ok());
        assert!(mmu.store16_le(&mut *tlb, BASE, 0xbeef).is_ok());

        assert!(mmu.load(&mut *tlb, BASE, &mut [0]).is_err());
        assert!(mmu.load_byte(&mut *tlb, BASE).is_err());
        assert!(mmu.load16_le(&mut *tlb, BASE).is_err());
        assert!(mmu.load32_le(&mut *tlb, BASE).is_err());
        assert!(mmu.load64_le(&mut *tlb, BASE).is_err());
...
fn byte_load_store_roundtrip() {
    ...
    TLB.with_borrow_mut(|tlb| {
        for i in 0..64 {
            mmu.store_byte(&mut *tlb, BASE.strict_add(i), pattern_byte(i))
                .unwrap();
        }

        for i in 0..64 {
            assert_eq!(
                mmu.load_byte(&mut *tlb, BASE.strict_add(i)).unwrap(),
                pattern_byte(i)
            );
        }
    });
...
fn slice_store_load_roundtrip_inside_one_page() {
    ...
    TLB.with_borrow_mut(|tlb| mmu.store(&mut *tlb, store_start, &data).unwrap());
...
fn slice_store_load_roundtrip_across_two_pages() {
    ...
    TLB.with_borrow_mut(|tlb| mmu.store(&mut *tlb, start, &data).unwrap());
...
fn slice_store_across_boundary_requires_write_on_both_pages_and_does_not_partially_write() {
    ...
    let byte = 0xFA;
    TLB.with_borrow_mut(|tlb| {
        mmu.store_byte(&mut *tlb, start, byte).unwrap();
        assert!(mmu.store(&mut *tlb, start, &[1, 2]).is_err());
    });
...
fn slice_load_across_boundary_requires_read_on_both_pages() {
    ...
    assert!(mmu.load(NoCache, start, &mut [0, 0]).is_err());
}
    ...
    TLB.with_borrow_mut(|tlb| {
        {
            ...
            assert_eq!(
                mmu.load16_le(&mut *tlb, off16).unwrap(),
...

        {
            ...
            assert_eq!(
                mmu.load32_le(&mut *tlb, off32).unwrap(),
...

        {
            ...
            assert_eq!(
                mmu.load64_le(&mut *tlb, off64).unwrap(),
...
fn scalar_stores_inside_one_page_are_little_endian() {
...

    {
        ...
        TLB.with_borrow_mut(|tlb| mmu.store16_le(&mut *tlb, off16, value).unwrap());
...

    {
        ...
        TLB.with_borrow_mut(|tlb| mmu.store32_le(&mut *tlb, off32, value).unwrap());
...

    {
        ...
        TLB.with_borrow_mut(|tlb| mmu.store64_le(&mut *tlb, off64, value).unwrap());
...
fn scalar_loads_crossing_page_boundary_read_expected_bytes() {
    ...
    TLB.with_borrow_mut(|tlb| {
        ...
        for bytes_in_first_page in 1..4 {
            ...
            assert_eq!(
                mmu.load32_le(&mut *tlb, start).unwrap(),
        ...
        for bytes_in_first_page in 1..8 {
            let start = base_end.strict_sub(bytes_in_first_page);
            assert_eq!(
                mmu.load64_le(&mut *tlb, start).unwrap(),
...
fn scalar_stores_crossing_page_boundary_write_expected_bytes() {
...

    {
        ...
        TLB.with_borrow_mut(|tlb| mmu.store16_le(&mut *tlb, off16, value16).unwrap());
    ...
    for bytes_in_first_page in 1..4_u8 {
        ...
        TLB.with_borrow_mut(|tlb| mmu.store32_le(&mut *tlb, start, value).unwrap());
        assert_eq!(
    ...
    for bytes_in_first_page in 1..8_u8 {
        ...
        TLB.with_borrow_mut(|tlb| mmu.store64_le(&mut *tlb, start, value).unwrap());
...
fn crossing_scalar_load_requires_read_on_both_pages() {
    ...
    assert!(mmu.load16_le(NoCache, PAGE_SIZE_U64.strict_sub(1)).is_err());
}
...
fn crossing_scalar_store_requires_write_on_both_pages() {
    ...
    assert!(
        mmu.store16_le(NoCache, PAGE_SIZE_U64.strict_sub(1), 0xbeef)
            .is_err()
    );
}
...
fn crossing_scalar_access_to_unmapped_second_page_faults() {
    ...
    TLB.with_borrow_mut(|tlb| {
        mmu.store_byte(&mut *tlb, last, 0xaa).unwrap();

        assert!(mmu.load16_le(&mut *tlb, last).is_err());
        assert!(mmu.store16_le(&mut *tlb, last, 0xbeef).is_err());

        assert_eq!(mmu.load_byte(&mut *tlb, last).unwrap(), 0xaa);
    });
}
...
fn store_byte_marks_executable_page_dirty() {
    ...
    TLB.with_borrow_mut(|tlb| mmu.store_byte(tlb, BASE, 0xaa).unwrap());
    assert!(is_dirty(&mmu, 0));
}
...
fn store_slice_marks_executable_page_dirty() {
    let mmu = new_iommu(1, MemProt::WRITE | MemProt::EXECUTE);
    assert!(!is_dirty(&mmu, 0));
    TLB.with_borrow_mut(|tlb| mmu.store(tlb, BASE.strict_add(8), &[1, 2, 3, 4]).unwrap());
    assert!(is_dirty(&mmu, 0));
}
...
fn store_slice_crossing_pages_marks_both_executable_pages_dirty() {
    ...
    TLB.with_borrow_mut(|tlb| {
        mmu.store(
            tlb,
            BASE.strict_add(PAGE_SIZE_U64.strict_sub(2)),
            &[1, 2, 3, 4],
        )
        .unwrap()
    });
...
fn single_page_scalar_store_marks_executable_page_dirty() {
    ...
    TLB.with_borrow_mut(|tlb| mmu.store64_le(tlb, BASE, 0x0123_4567_89ab_cdef).unwrap());
...
fn bug_load_exactly_one_page_should_not_require_next_page() {
    ...
    assert!(mmu.load_into_uninit(NoCache, BASE, &mut out).is_ok());
...
fn bug_store_exactly_one_page_should_not_require_next_page() {
    ...
    assert!(mmu.store(NoCache, BASE, &data).is_ok());
}
...
fn bug_cross_page_scalar_store_should_mark_touched_executable_pages_dirty() {
    ...
    TLB.with_borrow_mut(|tlb| mmu.store16_le(tlb, addr, 0xbeef).unwrap());
...
fn unmap_memory_unmaps_one_page() {
    ...
    TLB.with_borrow_mut(|tlb| {
        assert!(mmu.load_byte(&mut *tlb, BASE).is_err());
        assert!(mmu.store_byte(&mut *tlb, BASE, 0xbb).is_err());

        assert!(mmu.load(&mut *tlb, BASE, &mut [0]).is_err());
        assert!(mmu.store(&mut *tlb, BASE, &[0xcc]).is_err());
...
fn unmap_memory_unmaps_only_requested_page_range() {
    ...
    TLB.with_borrow_mut(|tlb| {
        mmu.store_byte(&mut *tlb, page_addr(0), 0x10).unwrap();
        mmu.store_byte(&mut *tlb, page_addr(1), 0x20).unwrap();
        mmu.store_byte(&mut *tlb, page_addr(2), 0x30).unwrap();
    });

    mmu.unmap(page_addr(1), page).unwrap();

    TLB.with_borrow_mut(|tlb| {
        assert_eq!(mmu.load_byte(&mut *tlb, page_addr(0)).unwrap(), 0x10);
        assert!(mmu.load_byte(&mut *tlb, page_addr(1)).is_err());
        assert_eq!(mmu.load_byte(&mut *tlb, page_addr(2)).unwrap(), 0x30);

        assert!(mmu.store_byte(&mut *tlb, page_addr(0), 0x11).is_ok());
        assert!(mmu.store_byte(&mut *tlb, page_addr(1), 0x21).is_err());
        assert!(mmu.store_byte(&mut *tlb, page_addr(2), 0x31).is_ok());
    });
}
...
fn unmap_memory_can_unmap_multiple_pages() {
    ...
    TLB.with_borrow_mut(|tlb| {
        assert!(mmu.load_byte(&mut *tlb, page_addr(0)).is_ok());
        assert!(mmu.load_byte(&mut *tlb, page_addr(1)).is_err());
        assert!(mmu.load_byte(&mut *tlb, page_addr(2)).is_err());
        assert!(mmu.load_byte(&mut *tlb, page_addr(3)).is_ok());
    });
}
...
fn unmap_memory_rejects_unaligned_start_and_leaves_mapping_unchanged() {
    ...
    TLB.with_borrow_mut(|tlb| mmu.store_byte(&mut *tlb, BASE, 0xaa).unwrap());

    assert!(mmu.unmap(1, page).is_err());

    TLB.with_borrow_mut(|tlb| {
        assert_eq!(mmu.load_byte(&mut *tlb, BASE).unwrap(), 0xaa);
        assert!(mmu.store_byte(&mut *tlb, BASE, 0xbb).is_ok());
    });
}
...
fn unmap_memory_already_unmapped_passes_and_is_noop() {
    ...
    TLB.with_borrow_mut(|tlb| {
        ...
        assert!(mmu.store_byte(&mut *tlb, BASE, 0xaa).is_ok());
    });
}
...
fn unmap_memory_rejects_address_overflow() {
    ...
    assert!(mmu.load_byte(NoCache, BASE).is_ok());
}
...
fn unmap_memory_then_remap_restores_access() {
    ...
    TLB.with_borrow_mut(|tlb| {
        assert!(mmu.store_byte(&mut *tlb, BASE, 0xff).is_err());
        assert!(mmu.load_byte(&mut *tlb, BASE).is_err());
    });
    ...
    TLB.with_borrow_mut(|tlb| {
        ...
        assert_eq!(mmu.load_byte(&mut *tlb, BASE).unwrap(), 0xbb);
    });
}
...
fn mem_protect_can_make_page_write_only() {
    ...
    TLB.with_borrow_mut(|tlb| {
        ...
        assert!(mmu.store_byte(&mut *tlb, BASE, 0xbb).is_ok());
        assert!(mmu.store(&mut *tlb, BASE, &[0xcc]).is_ok());
        assert!(mmu.store16_le(&mut *tlb, BASE, 0xbeef).is_ok());
    });
}
...
fn mem_protect_can_make_page_execute_only_for_data_accesses() {
    ...
    TLB.with_borrow_mut(|tlb| {
        ...
        assert!(mmu.load_into_uninit(&mut *tlb, BASE, &mut out).is_err());
        assert!(mmu.store(&mut *tlb, BASE, &[0xaa]).is_err());
    });
}
...
fn mem_protect_can_restore_read_write_after_read_only() {
    ...
    TLB.with_borrow_mut(|tlb| assert!(mmu.store_byte(&mut *tlb, BASE, 0xaa).is_err()));
...
fn mem_protect_only_changes_requested_pages() {
    ...
    TLB.with_borrow_mut(|tlb| {
        assert!(mmu.store_byte(&mut *tlb, page_addr(0), 0xaa).is_ok());
        assert!(mmu.store_byte(&mut *tlb, page_addr(1), 0xbb).is_err());

        assert!(mmu.load_byte(&mut *tlb, page_addr(0)).is_ok());
        assert!(mmu.load_byte(&mut *tlb, page_addr(1)).is_ok());
    });
}
...
fn mem_protect_can_change_multiple_pages() {
    ...
    TLB.with_borrow_mut(|tlb| {
        assert!(mmu.store_byte(&mut *tlb, page_addr(0), 0xaa).is_err());
        assert!(mmu.store_byte(&mut *tlb, page_addr(1), 0xbb).is_err());
        assert!(mmu.store_byte(&mut *tlb, page_addr(2), 0xcc).is_ok());

        assert!(mmu.load_byte(&mut *tlb, page_addr(0)).is_ok());
        assert!(mmu.load_byte(&mut *tlb, page_addr(1)).is_ok());
        assert!(mmu.load_byte(&mut *tlb, page_addr(2)).is_ok());
    });
}
...
fn mem_protect_rejects_sparse_range_and_does_not_partially_update() {
    ...
    TLB.with_borrow_mut(|tlb| {
        assert!(mmu.store_byte(&mut *tlb, page_addr(0), 0xaa).is_ok());
        assert!(mmu.store_byte(&mut *tlb, page_addr(2), 0xbb).is_ok());

        assert_eq!(mmu.load_byte(&mut *tlb, page_addr(0)).unwrap(), 0xaa);
        assert_eq!(mmu.load_byte(&mut *tlb, page_addr(2)).unwrap(), 0xbb);
    });
}
...
fn mem_protect_rejects_address_overflow() {
    ...
    assert!(mmu.store_byte(NoCache, BASE, 0xaa).is_ok());
}
...
fn unaligned_scalar_loads_inside_page_succeed() {
    ...
    TLB.with_borrow_mut(|tlb| {
        assert_eq!(
            mmu.load16_le(&mut *tlb, vaddr).unwrap(),
            u16::from_le_bytes(pattern_array::<2>(1))
        );

        assert_eq!(
            mmu.load32_le(&mut *tlb, vaddr).unwrap(),
            u32::from_le_bytes(pattern_array::<4>(1))
        );

        assert_eq!(
            mmu.load64_le(&mut *tlb, vaddr).unwrap(),
...
fn unaligned_scalar_stores_inside_page_succeed() {
    ...
    TLB.with_borrow_mut(|tlb| mmu.store16_le(&mut *tlb, addr16, 0xbeef).unwrap());
    assert_eq!(read_array::<2>(&mmu, addr16), 0xbeefu16.to_le_bytes());

    let addr32 = BASE.strict_add(3);
    TLB.with_borrow_mut(|tlb| mmu.store32_le(&mut *tlb, addr32, 0xaabb_ccdd).unwrap());
    assert_eq!(read_array::<4>(&mmu, addr32), 0xaabb_ccddu32.to_le_bytes());

    let addr64 = BASE.strict_add(5);
    TLB.with_borrow_mut(|tlb| {
        mmu.store64_le(&mut *tlb, addr64, 0x0123_4567_89ab_cdef)
            .unwrap()
    });
    assert_eq!(
...
fn failed_crossing_scalar_store_does_not_write_first_page_when_second_page_lacks_write() {
    ...
    TLB.with_borrow_mut(|tlb| {
        let before = mmu.load_byte(&mut *tlb, start).unwrap();

        assert!(mmu.store16_le(&mut *tlb, start, 0xbeef).is_err());

        assert_eq!(mmu.load_byte(&mut *tlb, start).unwrap(), before);
    });
}
...
fn failed_crossing_scalar_store_does_not_write_second_page_when_first_page_lacks_write() {
    ...
    TLB.with_borrow_mut(|tlb| {
        let second_page_before = mmu.load_byte(&mut *tlb, page_addr(1)).unwrap();

        assert!(mmu.store16_le(&mut *tlb, start, 0xbeef).is_err());

        assert_eq!(
            mmu.load_byte(&mut *tlb, page_addr(1)).unwrap(),
            second_page_before
        );
    });
}
...
fn scalar_accesses_near_u64_max_fault_instead_of_wrapping() {
    ...
    TLB.with_borrow_mut(|tlb| {
        assert!(mmu.load16_le(&mut *tlb, u64::MAX).is_err());
        assert!(mmu.load32_le(&mut *tlb, u64::MAX - 1).is_err());
        assert!(mmu.load64_le(&mut *tlb, u64::MAX - 3).is_err());

        assert!(mmu.store16_le(&mut *tlb, u64::MAX, 0xbeef).is_err());
        assert!(
            mmu.store32_le(&mut *tlb, u64::MAX - 1, 0xaabb_ccdd)
                .is_err()
        );
        assert!(
            mmu.store64_le(&mut *tlb, u64::MAX - 3, 0x0123_4567_89ab_cdef)
                .is_err()
        );
    });
}
...
fn fetch_aarch64_faults_on_non_execute_page() {
    let mmu = new_iommu(1, MemProt::READ | MemProt::WRITE);
    assert!(mmu.fetch_aarch64(NoCache, BASE).is_err());
}
...
fn fetch_aarch64_faults_on_unaligned_address() {
    let mmu = new_iommu(1, MemProt::EXECUTE);
    TLB.with_borrow_mut(|tlb| {
        assert!(mmu.fetch_aarch64(&mut *tlb, BASE.strict_add(1)).is_err());
        assert!(mmu.fetch_aarch64(&mut *tlb, BASE.strict_add(2)).is_err());
        assert!(mmu.fetch_aarch64(&mut *tlb, BASE.strict_add(3)).is_err());
    });
}
...
fn fetch_aarch64_returns_correct_little_endian_word() {
    ...
    TLB.with_borrow_mut(|tlb| {
        mmu.store(&mut *tlb, addr, &insn_bytes).unwrap();

        assert_eq!(
            mmu.fetch_aarch64(&mut *tlb, addr).unwrap(),
...
fn map_and_access_last_possible_page() {
    ...
    TLB.with_borrow_mut(|tlb| {
        ...
        mmu.store_byte(&mut *tlb, u64::MAX, 0xbb).unwrap();
        assert_eq!(mmu.load_byte(&mut *tlb, u64::MAX).unwrap(), 0xbb);

        assert!(mmu.fetch_aarch64(&mut *tlb, base).is_ok());
