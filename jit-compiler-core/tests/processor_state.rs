...
use jit_compiler_core::ir::{ExecIrBuilder, IConst, IntWidth, Terminator};
...
fn lower_width_x_register_loads_read_low_order_bits() {
    ...
    let diff8 = builder.isub(got8, expected8);
    ...
    let diff16 = builder.isub(got16, expected16);
    ...
    let diff32 = builder.isub(got32, expected32);
